...
cd
[ -e linux-$KERNEL_VERSION ] || wget -q -c https://mirrors.edge.kernel.org/pub/linux/kernel/v5.x/linux-$KERNEL_VERSION.tar.gz
[ -e linux-$KERNEL_VERSION ] || tar xzf linux-$KERNEL_VERSION.tar.gz
mv linux-$KERNEL_VERSION /build
...
## To allow root to dump kernel page table
# 5.x
echo 'CONFIG_X86_PTDUMP=y' >> linux-$KERNEL_VERSION/.config
echo 'CONFIG_X86_PTDUMP_CORE=y' >> linux-$KERNEL_VERSION/.config
# 6.x
